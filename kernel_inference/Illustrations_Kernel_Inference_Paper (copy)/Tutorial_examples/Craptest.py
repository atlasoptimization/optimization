...
x_data=x_measured[:,1]
x_realization=x_simu[:,1]
K_ij=C_gamma
K_t=K_estimated[:,sample_index.astype(int)]

x_hat=np.reshape(K_t@np.linalg.pinv(K_ij)@(x_data-np.reshape(mu_beta,[10]))+np.reshape(G@beta,[100]),[100])

fig3=plt.figure(dpi=200)

plt.scatter(t_sample,x_data)
plt.plot(t,x_realization)
plt.plot(t,x_hat)
