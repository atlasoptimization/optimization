...
sigma=1/wavelengths
P_true, T_true, U_true, C_true, D_true = 1013.25, 20, 0, 0, 50
...
x_0=np.array([1000,10,40])               #P,T,D
    
def loss_fun(x_vec):
    d_est_vec_opt=np.zeros([n_obs])
    for k in range(n_obs):
        d_est_vec_opt[k]=x_vec[2]*CiddorPy.CiddorHillGroup(sigma[k], x_vec[0], x_vec[1], U_true, C_true)
...
d_est_vec_opt=np.zeros([n_obs])
for k in range(n_obs):
    d_est_vec_opt[k]=x_opt[2]*CiddorPy.CiddorHillGroup(sigma[k], x_opt[0], x_opt[1], U_true, C_true)
