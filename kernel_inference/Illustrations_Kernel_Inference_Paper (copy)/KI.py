...
def Kernel_inference_inhomogeneous(X_list, Lambda_p, Psi_list,r, G_list="none", A="none", max_iter=100, tol=10**(-2)):
    ...
    #     obj_val_1=0
    #     for k in range(n_S_obs):
    #         temp_1=n_S_i[k]*(np.log(sf.Pseudodeterminant(eta_list_var[k],tol_obj))+np.trace(S_phi_list[k]@lina.pinv(eta_list_var[k],rcond=tol_obj,hermitian=True)))
    #         obj_val_1=obj_val_1+temp_1
    #     obj_val_2=r*(-np.log(sf.Pseudodeterminant(gamma_var,tol_obj)) + np.trace(Lambda_p_inv@gamma_var))
    
    #     Eigs=np.linalg.eigvalsh(gamma_var)
    #     bar_val=10**(10)*(np.sum((Eigs<-tol_obj).astype(int)))
    #     for k in range(n_S_obs):
    #         Eigs=np.linalg.eigvalsh(eta_list_var[k])
    #         temp_2=10**(20)*(np.sum((Eigs<-tol_obj).astype(int)))
