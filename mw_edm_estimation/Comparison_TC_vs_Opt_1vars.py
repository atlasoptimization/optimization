...
sigma=1/wavelengths
P_true, T_true, U_true, C_true, D_true = 1013.25, 20, 100, 0, 50
...
x_0=np.array([10])               #P,T,D
    
def loss_fun(x_vec):
    d_est_vec_opt=np.zeros([n_obs])
    for k in range(n_obs):
        d_est_vec_opt[k]=D_true*CiddorPy.CiddorHillGroup(sigma[k], P_true, x_vec[0], U_true, C_true)
...
d_est_vec_opt=np.zeros([n_obs])
for k in range(n_obs):
    d_est_vec_opt[k]=D_true*CiddorPy.CiddorHillGroup(sigma[k], P_true, x_opt[0], U_true, C_true)
